...
public class LoginController extends BaseController {
    ...
    LoginResponse handleLogin(@RequestBody Member member, HttpServletRequest request, HttpServletResponse response) {
        ...
        Condition condition=tech.codingclub.helix.tables.Members.MEMBERS.EMAIL.eq(member.email).and(tech.codingclub.helix.tables.Members.MEMBERS.PASSWORD.eq(member.password));

        List<Member> x= (List<Member>) GenericDB.getRows(tech.codingclub.helix.tables.Members.MEMBERS,Member.class, condition,1);
       if(x!=null && x.size()>0)
       {
           Member memberTemp=x.get(0);
           memberTemp.role="cm";
           ControllerUtils.setUserSession(request,memberTemp);
           return new LoginResponse(memberTemp.id,true,"Logedin successfully!");
       }
       else {
