...
public class ControllerUtils {
    ...
    public static Member getCurrentMember(HttpServletRequest request) {
        Member member = new GenericDB<Member>().getRow(tech.codingclub.helix.tables.Members.MEMBERS, Member.class, tech.codingclub.helix.tables.Members.MEMBERS.ID.eq(ControllerUtils.getUserId(request)));
        if (member != null) {
            ...
            if (cookies != null) {
                for (Cookie cookie : cookies) {
                    if (cookie.getName().equals(TOKEN_COOKIE)) {
                        Member member = new GenericDB<Member>().getRow(Tables.MEMBERS, Member.class, tech.codingclub.helix.tables.Members.MEMBERS.TOKEN.eq(cookie.getValue()));
    ...
    public static Boolean isUserAdmin(HttpServletRequest httpServletRequest) {
        ...
        if (cookies != null) {
            for (Cookie cookie : cookies) {
                if (cookie.getName().equals(TOKEN_COOKIE)) {
                    Member member = new GenericDB<Member>().getRow(Tables.MEMBERS, Member.class, tech.codingclub.helix.tables.Members.MEMBERS.TOKEN.eq(cookie.getValue()).and(tech.codingclub.helix.tables.Members.MEMBERS.ROLE.eq("cc-admin")));
                    if (member != null && member.token != null) { //.&& author.token_expire_time != null && author.token_expire_time > new Date().getTime()) {
                        setUserSession(httpServletRequest, member);
                        return true;
                    }
    ...
    public static Boolean isUserCodingMafia(HttpServletRequest httpServletRequest) {
        ...
        if (cookies != null) {
            for (Cookie cookie : cookies) {
                if (cookie.getName().equals(TOKEN_COOKIE)) {
                    Member member = new GenericDB<Member>().getRow(Tables.MEMBERS, Member.class, tech.codingclub.helix.tables.Members.MEMBERS.TOKEN.eq(cookie.getValue()).and(tech.codingclub.helix.tables.Members.MEMBERS.ROLE.eq("cm")));
                    if (member != null && member.token != null) { //.&& author.token_expire_time != null && author.token_expire_time > new Date().getTime()) {
                        setUserSession(httpServletRequest, member);
                        return true;
                    }
